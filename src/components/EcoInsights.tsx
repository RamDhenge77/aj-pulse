...
const equivalents = {
  ...
  money: [
    ...
    { label: "Month of DEWA Bill", ratio: 1070, emoji: "🏠" }, // AED
...
const getEquivalentInsights = (
  ...
  return equivalents[type]
    .map((eq) => {
      const quantity = Math.floor(value / eq.ratio);
      return quantity > 0
        ? {
            text: `${quantity} ${eq.label}`, // This now correctly formats as "5 Trees Planted" without the X
            emoji: eq.emoji,
          }
        : null;
    })
...
const EcoInsights: React.FC<EcoInsightsProps> = ({ totalTrips = 0 }) => {
  ...
  useEffect(() => {
    ...
    console.log("Starting autoplay for", hoveredCard);
